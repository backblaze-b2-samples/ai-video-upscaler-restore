...
## Purpose
List, preview, download, and delete **every** object in the Backblaze B2 bucket — across all prefixes (`uploads/`, `jobs/`, `restored/`). This is the full-bucket explorer; for a `restored/`-scoped, source-paired view see the [Before/After Browser](before-after-browser.md).
