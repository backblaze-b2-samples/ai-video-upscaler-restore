...
def get_s3_client():
    return boto3.client(
        ...
        config=Config(
            ...
            user_agent_extra="b2ai-video-upscaler-restore",
        ),
    )
...
def put_object_bytes(data: bytes, key: str, content_type: str) -> int:
    """Upload raw bytes to a key. Returns the byte count.

    Used for restored outputs and JSON manifests — both are produced
    server-side rather than streamed from a client upload.
    """
    client = get_s3_client()
    try:
        client.put_object(
...
def list_object_keys(prefix: str, max_keys: int = 1000) -> list[dict]:
    """List raw object descriptors (key/size/last_modified) under a prefix.

    Paginates fully. Returns lightweight dicts rather than FileMetadata so
    callers that just need keys/sizes (manifest enumeration, prefix delete)
    don't pay for MIME guessing.
    """
    client = get_s3_client()
    out: list[dict] = []
    kwargs: dict = {
...
def delete_prefix(prefix: str) -> int:
    ...
    Scoped strictly to the given prefix — used to clean up a job's own
    restored/<id>/ outputs without ever touching other prefixes.
    """
    if not prefix:
        raise ValueError("delete_prefix requires a non-empty prefix")
    objs = list_object_keys(prefix)
    for obj in objs:
        delete_file(obj["key"])
    return len(objs)
