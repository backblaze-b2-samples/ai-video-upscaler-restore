...
// --- Restoration Jobs (primary entity) ---

export async function getJobs() {
  return apiFetch<RestorationJob[]>("/jobs");
}
...
export async function getRestorations() {
  return apiFetch<RestorationItem[]>("/jobs/restorations");
}
...
export async function updateJob(id: string, payload: JobUpdate) {
  return apiFetch<RestorationJob>(`/jobs/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function deleteJob(id: string) {
  return apiFetch<{ deleted: boolean; id: string }>(`/jobs/${id}`, {
    method: "DELETE",
  });
}

export async function runJob(id: string) {
  return apiFetch<RestorationJob>(`/jobs/${id}/run`, { method: "POST" });
}
