...
## Two-step install (read this first)

The backend has two requirements files:

- `services/api/requirements.txt` — **base**. Everything for the API, the UI, all B2 features, tests, and lint. Install this for development.
- `services/api/requirements-restore.txt` — **engine**. The heavy ML stack (torch, realesrgan, gfpgan, basicsr, opencv, imageio-ffmpeg). Needed **only to actually RUN a restoration**.

The engine is lazy-imported inside functions in `app/repo/upscaler.py` and `app/repo/video.py`, so `from main import app`, `pnpm test:api`, `pnpm lint:api`, and `pnpm build` all pass with **base deps only**. **Restoration tests mock the engine** (`tests/test_restoration.py` monkeypatches `upscale_image` / `extract_frames` / `assemble_video`; test images are generated in-memory with PIL — no binary fixtures). Do **not** `pip install -r requirements-restore.txt` just to run the suite, and do not import torch/realesrgan at module top level.
...
- E2E: `pnpm test:e2e` (run `pnpm --filter @ai-video-upscaler-restore/web exec playwright install chromium` once first)
...
const { data: detail } = useJob(id);     // polls every 2s while running
const { data: stats } = useJobStats();
const { data: files } = useFiles(prefix, limit);  // full-bucket explorer
```
