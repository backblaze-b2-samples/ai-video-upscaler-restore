...
# Feature: File Upload (Ingest source archive)

## Purpose
Ingest the source archive — degraded or low-resolution footage and images — from the browser into `uploads/` on Backblaze B2 with real-time progress tracking. These uploads become the selectable sources when creating a Restoration Job.

## Used By
- UI: `/upload` page (titled "Ingest source archive"), upload form component
- API: `POST /upload`
- Downstream: a [Restoration Job](restoration-jobs.md) picks an `uploads/` object as its source
